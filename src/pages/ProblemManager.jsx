            ...
            (currentQuestion == null && questionQueue.length != 0) 
                ...
                <div className='problem-start'>
                    <div className='center-align'>
                        <h1>{questionsObject && questionsObject[questionQueue[0]] && questionsObject[questionQueue[0]]['Question Name']}</h1>
                        <h2>Language: {questionsObject && questionsObject[questionQueue[0]] && questionsObject[questionQueue[0]]['Language']}</h2>
                        <h2>Type: {questionsObject && questionsObject[questionQueue[0]] && questionsObject[questionQueue[0]]['Question Type']}</h2>
                        <h2>Difficulty: {questionsObject && questionsObject[questionQueue[0]] && questionsObject[questionQueue[0]]['Question Difficulty']}</h2>

                        <button className='problem-button' onClick={() => this.selectQuestion(questionQueue)}><span>START</span></button>
                    </div>
                </div>
            
            : // if a question is already active
            currentQuestion != null ? <Problem/>
            
            : // if there are no selected questions
            <div className='problem-start'>
                <div className='center-align'>
                    <h1>No questions remaining</h1>
                    <NavLink className="no-text-decoration" activeClassName={'no-text-decoration'} to={'/'} >
                        <button className='problem-button' onClick={() => this.returnHome()}>
                            <span>RETURN</span>
                        </button>
                    </NavLink>
                </div>
